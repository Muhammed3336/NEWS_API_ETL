...
📂 Project Files
lambda.py — AWS Lambda function code that fetches news from the News API and pushes data to Kafka on EC2.
glue.py — AWS Glue script that reads news data from Kafka, performs sentiment analysis, and writes results to S3.
code.sql — SQL script used to create tables and load data into Snowflake from S3.
