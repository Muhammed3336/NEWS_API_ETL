...
def lambda_handler(event, context):
    # News API Key
    API_KEY = os.environ.get("kafkanewapi",)
    if not API_KEY:
        return {"statusCode": 500, "body": "Missing kafkanewapi"}
    ...
    try:
        producer = KafkaProducer(
            bootstrap_servers=os.environ.get("KAFKA_BROKER", "13.200.237.122:9092"),  # Use EC2 IP
            value_serializer=lambda v: json.dumps(v).encode("utf-8")
        )
    except Exception as e:
        return {"statusCode": 500, "body": f"Kafka connection failed: {str(e)}"}

    topic = os.environ.get("KAFKA_TOPIC", "mutopic")
